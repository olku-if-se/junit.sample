...
abstract class AbstractMatcher {
     ...
     * @return <code>true</code> if cursor is not null after the operation
     */
    final boolean safeNextIs(final int opcode) {
        nextIs(opcode);
        return cursor != null;
    }
     ...
     *
     * @param opcode expected opcode
     * @param desc   expected descriptor
     * @return <code>true</code> if cursor is not null after the operation
     */
    final boolean safeNextIsType(final int opcode, final String desc) {
        nextIsType(opcode, desc);
        return cursor != null;
    }
     ...
     * @param descriptor expected method descriptor
     * @return <code>true</code> if cursor is not null after the operation
     */
    final boolean safeNextIsInvoke(final int opcode, final String owner,
                                   final String name, final String descriptor) {
        nextIsInvoke(opcode, owner, name, descriptor);
        return cursor != null;
    }
